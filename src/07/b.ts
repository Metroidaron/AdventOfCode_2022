...
let USED_SPACE = 47870454;
const UPDATE_SIZE = 30000000;
let SPACE_NEEDED = 7870454;
...
USED_SPACE = sumDirChildrenSizeRecursively(fileSystem['/'] as iDirectory);
console.log(USED_SPACE);
console.log();

console.log('Free space on Device');
console.log(MAX_SPACE - USED_SPACE); // 22,129,546
console.log();

console.log('Need to Free this amount of space for the update');
SPACE_NEEDED = UPDATE_SIZE - (MAX_SPACE - USED_SPACE)
console.log(SPACE_NEEDED); // 7,870,454
console.log(); 

let candidates: { [key: string]: number } = {};
Object.keys(allUniqueDirectories).forEach(key => {
  if(allUniqueDirectories[key] > SPACE_NEEDED) candidates[key] = allUniqueDirectories[key];
});

let lowest = MAX_SPACE;
Object.keys(candidates).forEach(i => {
  if(candidates[i] < lowest) lowest = candidates[i];
});

console.log('The Smallest Dir we can delete and have enough space for the update');
console.log(lowest);
console.log(); 
